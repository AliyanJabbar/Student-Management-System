...
class Student {
  ...
  addBalance(amount: number) {
    ...
    console.log(
      chalk.green(
        `BALANCE HAS BEEN INCREASED BY ${chalk.magenta(
          amount
        )}, NEW BALANCE IS: ${chalk.magenta(this.balance)}`
      )
    );
  }
  ...
  payTuitionFees(amount: number) {
    if (amount > this.balance) {
      console.log(
        chalk.red(
          "INSUFFICIENT BALANCE!, PLEASE ADD SOME MONEY IN YOUR BALANCE..."
        )
      );
    } else {
      this.balance -= amount;
  ...
  showStatus() {
    ...
    console.log(
      chalk.green(`COURSES ENROLLED: ${chalk.magenta(this.courses)}`)
    );
  }
}
...
class Student_Management {
  ...
  add_student(name: string) {
    ...
    console.log(
      chalk.green(`STUDENT ${chalk.magenta(name)} HAS BEEN ADDED SUCCESSFULLY!`)
    );
  }
  ...
  enroll_student(id: number, course: string) {
    ...
    if (student) {
      ...
      console.log(
        `STUDENT ${chalk.magenta(
          student.name
        )} HAS BEEN ENROLLED IN ${chalk.magenta(course)} SUCCESSFULLY!`
      );
    } else {
  ...
  // for add balance
  add_Balance(id: number, Amount: number) {
    let student = this.find_Student(id);
  ...
  pay_TuitionFees(id: number, amount: number) {
    let student = this.find_Student(id);
    if (student) {
...
async function main() {
  console.log(
    "-".repeat(25),
    chalk.bgMagenta("WELCOME TO STUDENT MANAGEMENT SYSTEM!!!"),
    "-".repeat(25)
  );
      ...
      if (std.students.length === 0) {
        ...
        let showingStatus = await inquirer.prompt({
          ...
          message: chalk.yellow(
            "INPUT STUDENT'S ID WHOSE STATUS YOU WANT TO SEE..."
          ),
        });
    ...
    if(operation.select === "ADD BALANCE"){
      if (std.students.length === 0) {
        ...
        let adding = await inquirer.prompt({
          ...
          type: "number",
          message: chalk.yellow(
            "INPUT STUDENT'S ID WHOSE BALANCE YOU WANT TO INCREASE..."
          ),
        });
    ...
    if (operation.select === "VIEW BALANCE") {
      if (std.students.length === 0) {
        ...
        let viewingBalance = await inquirer.prompt({
          ...
          type: "number",
          message: chalk.yellow(
            "INPUT STUDENT'S ID WHOSE BALANCE YOU WANT TO SEE..."
          ),
        });
    ...
    if (operation.select === "ENROLL IN COURSE") {
      if (std.students.length === 0) {
        ...
        let enrolling = await inquirer.prompt({
          ...
          type: "number",
          message: chalk.yellow(
            "INPUT STUDENT'S ID YOU WANT TO ENROLL IN A COURSE..."
          ),
        });
